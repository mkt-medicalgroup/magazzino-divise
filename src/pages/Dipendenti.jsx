...
export default function Dipendenti() {
  ...
  const [aperto, setAperto] = useState(null) // id dipendente con storico espanso
  const [storico, setStorico] = useState({})

  const vuoto = { nome: '', cognome: '', sede_id: '' }
  const [form, setForm] = useState(vuoto)
  const [nuovaSede, setNuovaSede] = useState('')

  async function load() {
    setLoading(true)
    const [{ data: dip, error: dipErr }, { data: sd }] = await Promise.all([
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h2>Dipendenti</h2>
          <p className="sub">Elenco dipendenti diviso per sede, con storico divise assegnate.</p>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <details style={{ marginTop: 16 }}>
          <summary style={{ cursor: 'pointer', fontSize: 13, color: 'var(--graphite)' }}>Gestisci sedi</summary>
          <form onSubmit={aggiungiSede} style={{ display: 'flex', gap: 8, marginTop: 12, alignItems: 'flex-end' }}>
              ...
              <tbody>
                {lista.map(d => (
                  <>
                    ...
                    {aperto === d.id && (
                      <tr>
                        <td colSpan={3} style={{ background: 'var(--canvas)' }}>
                          {!storico[d.id] ? (
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, padding: '6px 0' }}>
                              {storico[d.id].filter(m => m.tipo === 'Scarico').map(m => (
                                <div key={m.id} style={{ display: 'flex', gap: 10, alignItems: 'center', fontSize: 13 }}>
